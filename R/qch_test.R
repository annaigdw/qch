...
###################################################################
#' Perform composite hypothesis testing with FDR control.
#' 
...
#' @param Hconfig.H1 An integer vector (or a list of such vector) of the Hconfig index corresponding to the H1 configuration(s). Can be generated by the [GetH1AtLeast()] or [GetH1Equal()] functions.
#' If NULL, the tests of being associated with "at least q" number of traits/studies, for q=1,..Q are performed.  
#' @param Alpha the nominal Type I error rate for FDR control. Default is 0.05.
#'
#' @return A list of 3 objects 'Rejection', 'lFDR' and 'Pvalues'.
...
qch.test <- function(res.qch.fit,Hconfig,Hconfig.H1=NULL,Alpha=0.05){
  ...
  if(!is.null(res.qch.fit$posterior)){
    ...
    ### localFDR
    Tau1.list <- map(Hconfig.H1, ~fHconfig_sum_update_gaussian_copula_ptr_parallel(Hconfig = Hconfig[.x],
                                                                                        ...
                                                                                        Rinv = RcopulaInv)/res.qch.fit$fHconfig_sum) 
  }else{
    n <- nrow(res.qch.fit$f0Mat)
    Logf0Mat <- log(res.qch.fit$f0Mat)
    Logf1Mat <- log(res.qch.fit$f1Mat)
    ...
    ### localFDR
    Tau1.list <- map(Hconfig.H1, ~fHconfig_sum_update_ptr_parallel(Hconfig = Hconfig[.x],
                                                                        NewPrior = res.qch.fit$prior[.x],
                                                                        Logf0Mat = Logf0Mat,
                                                                        Logf1Mat = Logf1Mat)/res.qch.fit$fHconfig_sum) 
    
  }
  
  Order.list <-map(1:nb_test, ~order(Tau1.list[[.x]],decreasing = TRUE))
  FDR.list <- map(1:nb_test,~((1:n) - cumsum(Tau1.list[[.x]][Order.list[[.x]]]))/(1:n))
  ...
  EspTau0.list <- map(1:nb_test, ~(((1:n) - cumsum(Tau1.list[[.x]][Order.list[[.x]]]))/(Pi0.vec[.x]*n)))
