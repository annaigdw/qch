...
#'
#' @param Logf0Mat a matrix containing the \eqn{\log(f_0(x^i_q))}
#' @param Logf1Mat a matrix containing the \eqn{\log(f_1(x^i_q))}
#' @param Prior.init  the initialization of prior probabilities for each of the H-configurations.
...
#'
#' @param Hconfig A list of all possible combination of \eqn{H_0} and \eqn{H_1} hypotheses generated by the [GetHconfig()] function.
#' @param F0Mat a matrix containing the evaluation of the marginal cdf under \eqn{H_0} at each items, each column corresponding to a p-value serie.
#' @param F1Mat a matrix containing the evaluation of the marginal cdf under \eqn{H_1} at each items, each column corresponding to a p-value serie.
#' @param fHconfig a matrix containing H-config densities evaluated at each items, each column corresponding to a configurations.
...
#'
#' @return A numeric vector, each coordinate \eqn{i} corresponding to the evaluation of the Gaussian copula density function at observation \eqn{\code{zeta}_i}.
##
...
#' @param R the correlation matrix.
#'
#' @return A matrix containing the evaluation of the Gaussian density function for each H-configuration in columns.
##
...
Copula.Hconfig_gaussian_density <- function(Hconfig, F0Mat, F1Mat, R) {
  ...
  Copula.Hconfig <- purrr::map_dfc(Hconfig, function(h) {
    ...
    if (length(which(h == 1)) > 0) {
      zeta[, which(h == 1)] <- zeta1[, which(h == 1)]
    }
    if (length(which(h == 0)) > 0) {
      zeta[, which(h == 0)] <- zeta0[, which(h == 0)]
    }
...
#'
#' @param Hconfig A list of all possible combination of \eqn{H_0} and \eqn{H_1} hypotheses generated by the [GetHconfig()] function.
#' @param zeta0 a matrix containing the \eqn{\Phi(F_0(Z_{iq}))}, each column corresponding to a p-value serie.
#' @param zeta1 a matrix containing the \eqn{\Phi(F_1(Z_{iq}))}, each column corresponding to a p-value serie.
...
#'
#' @param Hconfig A list of all possible combination of \eqn{H_0} and \eqn{H_1} hypotheses generated by the [GetHconfig()] function.
#' @param fHconfig_sum a vector containing \eqn{\sum_c(w_c*psi_c)} for each items.
#' @param OldPrior  a vector containing the prior probabilities for each of the H-configurations.
#' @param Logf0Mat a matrix containing \eqn{\log(f_0Mat)}, each column corresponding to a p-value serie.
#' @param Logf1Mat a matrix containing \eqn{\log(f_1Mat)}, each column corresponding to a p-value serie.
#' @param zeta0 a matrix containing \code{qnorm(F0Mat)}, each column corresponding to a p-value serie.
#' @param zeta1 a matrix containing \code{qnorm(F1Mat)}, each column corresponding to a p-value serie.
...
R.MLE.memory <- function(Hconfig, fHconfig_sum, OldPrior, Logf0Mat, Logf1Mat, zeta0, zeta1, OldR, OldRinv) {
  ...
  for (c in 1:length(Hconfig)) {
    ...
    f_c <- rep(0, n)
    if (length(which(h == 1)) > 0) {
      f_c <- f_c + rowSums(Logf1Mat[, which(h == 1), drop = FALSE])
    }
    if (length(which(h == 0)) > 0) {
      f_c <- f_c + rowSums(Logf0Mat[, which(h == 0), drop = FALSE])
    }
    f_c <- exp(f_c)
